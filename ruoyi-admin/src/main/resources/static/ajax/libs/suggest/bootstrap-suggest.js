 ...
 * @desc    这是一个基于 bootstrap 按钮式下拉菜单组件的查询建议插件，必须使用于按钮式下拉菜单组件上。
...
(function (factory) {
    ...
    /**
     * 判断字段名是否在 options.searchFields查询字段配置中
     */
    ...
    function isInWord(keyword, key, value, options) {
        ...
        return value &&
            (inEffectiveFields(key, options) || inSearchFields(key, options)) && // 必须在有效的查询字段中
            (
    ...
    var defaultOptions = {
        ...
        ignorecase: FALSE,              // 前端查询匹配时，是否忽略大小写
        effectiveFields: [],            // 有效显示于列表中的字段，非有效字段都会过滤，默认全部有效。
        effectiveFieldsAlias: {},       // 有效字段的别名对象，用于 header 的显示
        searchFields: [],               // 有效查询字段，从前端查询过滤数据时使用，但不一定显示在列表中。effectiveFields 配置字段也会用于查询过滤
        twoWayMatch: TRUE,              // 是否双向匹配查询。为 true 即输入关键字包含或包含于匹配字段均认为匹配成功，为 false 则输入关键字包含于匹配字段认为匹配成功
        multiWord: FALSE,               // 以分隔符号分割的多关键字支持
        separator: ',',                 // 多关键字支持时的分隔符，默认为半角逗号
        delay: 300,                     //查询触发的延时时间间隔，单位毫秒
        emptyTip: '',                   // 查询为空时显示的内容，可为 html
        searchingTip: '查询中...',       // ajax查询时显示的提示内容，当查询时间较长时给出正在查询的提示
        ...
        fnPreprocessKeyword: null,      //查询过滤数据前，对输入关键字作进一步处理方法。注意，应返回字符串
